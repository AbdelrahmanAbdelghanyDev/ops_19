...
class ResConfigSettings(models.TransientModel):
    ...
    @api.constrains('module_cost_estimation_accounting')
    def show_stuff(self):
        for rec in self.env['cost.estimation'].search([]):
            rec.accounting_installed = self.module_cost_estimation_accounting

    @api.constrains('quotation_restriction')
    def hide_quotation(self):
        for rec in self.env['crm.lead'].search([]):
            rec.quotation_restriction = self.quotation_restriction
